    ...
    <table className="table table-bordered table-striped table-hover">
      ...
      <tbody>
        ...
        {uniqueCategories.map(category => {
          ...
          const futureCategoryEvents = eventos.filter(
            event => event.category === category && event.date >= '2023-03-10'
          );
          // Calcular ganancias totales para eventos futuros de la categoría actual
          const futureTotalRevenue = futureCategoryEvents.reduce((sum, event) => sum + event.revenue, 0);
          // Calcular la capacidad total para eventos futuros de la categoría actual
          const futureTotalCapacity = futureCategoryEvents.reduce((sum, event) => sum + event.capacity, 0);
          // Calcular la asistencia total para eventos futuros de la categoría actual
          const futureTotalAttendance = futureCategoryEvents.reduce((sum, event) => sum + event.assistance, 0);
      ...
      <tbody>
        ...
        {uniqueCategories.map(category => {
          ...
          const pastTotalRevenue = pastCategoryEvents.reduce((sum, event) => sum + event.revenue, 0);
          // Calcular la capacidad total para eventos pasados de la categoría actual
          const pastTotalCapacity = pastCategoryEvents.reduce((sum, event) => sum + event.capacity, 0);
          // Calcular la asistencia total para eventos pasados de la categoría actual
          const pastTotalAttendance = pastCategoryEvents.reduce((sum, event) => sum + event.assistance, 0);
